...
class igra:
    def __init__(self, master):
        self.canvas = Canvas(master, width=700, height=600, bg = "white")
        ...
        self.canvas.create_line([(500,0),(500,600)])
        self.canvas.create_line([(400,0),(400,600)])
        self.canvas.create_line([(300,0),(300,600)])
        self.canvas.create_line([(200,0),(200,600)])
        self.canvas.create_line([(100,0),(100,600)])
        ...
        self.canvas.create_line([(0,500),(700,500)])
        ...
        if not(self.konec):
            if self.navrsti == 0:
                ...
                if naredi:
                    self.matrika[b][a]=1
                    self.navrsti = 1
                    self.canvas.create_line([(100*a,100*b),(100*(a+1),100*(b+1))])
                    self.canvas.create_line([(100*a,100*(b+1)),(100*(a+1),100*b)])
                ...
                if naredi:
                    self.matrika[b][a]=(-1)
                    self.navrsti = 0
                    self.canvas.create_oval(100*a,100*b,100*(a+1),100*(b+1))
            for i in range(4):
                if i==0:  #vodoravno
                    for z in range(6):
                        for q in range(4):
                                ...
                                if self.matrika[z][q] == self.matrika[z][q+1]:
                                    if self.matrika[z][q] == self.matrika[z][q+2]:
                                        if self.matrika[z][q] == self.matrika[z][q+3]:
                                            self.konec=True
                                            self.canvas.create_line([(50 + 100*q,50 + 100*z),(50 + 100*(q+3),50 +100*z)])
                                ...
                                if self.matrika[z][q] == self.matrika[z+1][q+1]:
                                    if self.matrika[z][q] == self.matrika[z+2][q+2]:
                                        if self.matrika[z][q] == self.matrika[z+3][q+3]:
                                            self.konec=True
                                            self.canvas.create_line([(50 + 100*q,50 + 100*z),(50 + 100*(q+3),50 +100*(z+3))])
                elif i==2:  #navpicno
                    for z in range(3):
                        for q in range(7):
                            if not(self.matrika[z][q]==0):
                                if self.matrika[z][q] == self.matrika[z+1][q]:
                                    if self.matrika[z][q] == self.matrika[z+2][q]:
                                        if self.matrika[z][q] == self.matrika[z+3][q]:
                                            self.konec=True
                                            self.canvas.create_line([(50 + 100*q,50 + 100*z),(50 + 100*q,50 +100*(z+3))])
                else:
                    for z in range(3,6):
                        for q in range(4):
                            if not(self.matrika[z][q]==0):
